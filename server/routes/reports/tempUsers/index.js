...
router.route('/').get(async (req, res) => {
    try {
        ...
        const usersResults = await models.sequelize.query(
            'select * from cqc."AllUsers"',
            {
              type: models.sequelize.QueryTypes.SELECT
            }
        );

        if (usersResults && Array.isArray(usersResults)) {
        ...
        console.error('report/tempUsers - failed', err);
        return res.status(503).send();
    }
});
